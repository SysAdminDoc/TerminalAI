...
struct Entry {
    ...
    grid: TerminalGrid,
    /// Reads `OSC 9;4` out of this session's output. Stateful because the
    /// sequence routinely arrives split across two pty reads.
    progress: crate::progress::ProgressScanner,
        ...
        for stored in snapshot.sessions {
            ...
            state.entries.insert(
                ...
                Entry {
                    ...
                    scrollback,
                    grid,
                    // Deliberately not fed the restored history: a progress bar
                    // is about what a session is doing now, and a restored row
                    // is not running.
