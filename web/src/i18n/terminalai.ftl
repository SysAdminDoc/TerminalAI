...
launcher-subtitle = Preview the exact command before it starts.
...
rollup-subtitle = See spend and token coverage across the fleet.
explainer-title = How the fleet screen works
explainer-subtitle = A quick tour of attention, focus, and safety boundaries.
...
explainer-monitor-heading = Monitor and ship
explainer-monitor = Keep the focused terminal beside the fleet, watch cost and dwell, and move completed work through review with its context intact.
explainer-states-heading = What the states mean
explainer-safety-heading = Safety boundaries
explainer-safety = Approvals stay explicit, broadcasts show who is eligible before sending, and limits refuse new work without terminating sessions already running.
explainer-reading-heading = Reading the fleet
explainer-reading = Task, progress, dwell, cost, and the last meaningful line tell you where attention belongs before you open terminal output.
...
empty-demo-button = Explore safe demo
first-run-eyebrow = FIRST RUN
first-run-title = Three steps to your first controlled session
first-run-project = Register a project
first-run-project-detail = Choose a repository root; projects are discovered locally.
first-run-demo = Explore the safe demo
first-run-demo-detail = See every attention state without starting an agent.
first-run-launcher = Open the real launcher
first-run-launcher-detail = Preview the exact command before you run it.
first-run-progress = { $done } of { $total } complete
first-run-step-next = next
first-run-step-done = done
demo-mode-eyebrow = SAFE DEMO
demo-mode-title = Sample fleet only
demo-mode-detail = No agent process is running and no command can be sent.
demo-exit = Exit demo
demo-terminal-header = TerminalAI safe demo · frontend-only sample
demo-terminal-focus = Focus another row to compare its state.
demo-terminal-status = Current sample state: { $status }
demo-terminal-note = This preview never starts an agent or writes to a repository.
demo-read-only = Safe demo controls do not send commands.
demo-status-coverage = Safe demo loaded { $count } fleet states
...
projects-subtitle = Scan registered roots, then decide what to run next.
...
prompts-subtitle = Keep reusable instructions ready for repeatable work.
...
queue-subtitle = Stage the next instruction without interrupting the current run.
...
queue-pause-budget_exhausted = its session budget is spent
...
history-subtitle = Finished sessions and recoverable worktrees, kept local.
...
launcher-agent-option-claude = Claude Code
launcher-agent-option-codex = Codex
launcher-name = Session label
launcher-name-hint = optional
launcher-name-placeholder = e.g. API rewrite
launcher-cwd-placeholder = C:\Users\me\repos\project
label-known-projects-hint = from your registered roots
template-label-hint = declared in the repository
launcher-prompt = Initial prompt
launcher-prompt-hint = optional
launcher-prompt-placeholder = The first instruction sent to the agent
launcher-preset-placeholder = Preset name
# The advanced disclosure. Every label, hint, option, placeholder and button
# caption in it was a literal until 2026-08-08, so the coverage check --
# which reads the catalog and the `data-i18n` attributes -- reported clean
# over strings it could not see.
launcher-model = Model
launcher-model-hint = runtime detected; free text
launcher-model-placeholder = Detected at launch
launcher-effort = Reasoning effort
launcher-effort-hint = runtime detected; free text
launcher-effort-placeholder = Detected at launch
launcher-permission = Permission mode
launcher-permission-option-ask = Ask before risky actions
launcher-permission-option-plan = Plan only
launcher-permission-option-accept-edits = Accept edits
launcher-permission-option-bypass = Bypass permissions
launcher-sandbox = Sandbox
launcher-sandbox-option-read-only = Read only
launcher-sandbox-option-workspace-write = Workspace write
launcher-sandbox-option-danger-full-access = Danger full access
launcher-profile = Config profile
launcher-profile-hint = optional
launcher-profile-placeholder = work
launcher-resume = Resume
launcher-resume-option-new = New session
launcher-resume-option-last = Resume last
launcher-resume-option-session = Resume by id
launcher-resume-option-fork = Fork by id
launcher-resume-id = Native session id
launcher-resume-id-placeholder = session UUID
launcher-budget = Spend cap
launcher-budget-hint = USD, optional — held by TerminalAI, not the agent
launcher-budget-placeholder = No cap
launcher-search = Enable web search
launcher-extra-dirs = Extra writable folders
launcher-extra-dirs-placeholder = None selected
launcher-pick-extra = Add folders
launcher-worktree = Isolate in its own Git worktree
launcher-worktree-hint = private checkout and branch, so two sessions on one repo cannot see each other's edits
launcher-subagents = Concurrent subagents
launcher-subagents-hint = optional — the agent's own default is 20
launcher-subagents-placeholder = Agent default
launcher-agent-teams = Agent teams
launcher-agent-teams-hint = each teammate is a separate agent, and costs like one
launcher-agent-teams-option-default = Leave to the agent's configuration
launcher-agent-teams-option-on = Allow teams
launcher-agent-teams-option-off = Refuse teams
launcher-port-count = Port slots
launcher-port-count-hint = per session
launcher-port-base = Port base
launcher-port-base-hint = deterministic
launcher-agent-home = Agent config directory
launcher-agent-home-hint = optional; a different directory is a different account
launcher-agent-home-placeholder = Leave empty to use the signed-in account
launcher-env-passthrough = Inherit these variables
launcher-env-passthrough-hint = optional; comma separated names, nothing is inherited by default
launcher-env-passthrough-placeholder = e.g. ANTHROPIC_API_KEY, NODE_EXTRA_CA_CERTS
launcher-allowed-tools = Tools allowed without asking
launcher-allowed-tools-hint = optional; comma separated, e.g. Read, Bash(git log:*)
launcher-allowed-tools-placeholder = Leave empty to ask as usual
launcher-disallowed-tools = Tools this session may not use
launcher-disallowed-tools-hint = optional; comma separated
launcher-disallowed-tools-placeholder = e.g. WebFetch
launcher-settings = Extra settings
launcher-settings-hint = file path or JSON
launcher-settings-placeholder = Leave empty for the agent's own
launcher-setting-sources = Setting sources
launcher-setting-sources-hint = comma separated
launcher-setting-sources-placeholder = user, project, local
launcher-mcp-config = MCP server config
launcher-mcp-config-hint = optional; comma separated files or JSON
launcher-mcp-config-placeholder = Leave empty to use the agent's own servers
launcher-strict-mcp = Use only those MCP servers
launcher-plugin-dirs = Session plugins
launcher-plugin-dirs-hint = optional; comma separated directories
launcher-plugin-dirs-placeholder = e.g. C:\tools\my-plugin
launcher-plugin-urls = Session plugin URLs
launcher-plugin-urls-hint = optional; http(s) only — this fetches and runs remote code
launcher-plugin-urls-placeholder = https://example.com/plugin.zip
launcher-setup-hook = Setup hook
launcher-setup-hook-hint = optional local code
launcher-setup-hook-placeholder = e.g. pnpm install --frozen-lockfile
launcher-teardown-hook = Teardown hook
launcher-teardown-hook-hint = optional local code
launcher-teardown-hook-placeholder = e.g. pnpm run env:down
...
worktrees-subtitle = Review what can be safely removed before reclaiming it.
...
broadcast-subtitle = Send one instruction to eligible sessions; refusals stay visible.
...
broadcast-skip-budget = its session budget is spent — raise it or send explicitly
...
rollup-empty = No sessions have reported spend or token usage yet.
...
settings-subtitle = Set guardrails that refuse new work without interrupting live sessions.
settings-usage-title = Current fleet limit usage
...
settings-max-live-detail = Maximum number of sessions allowed to be running.
settings-default-budget = Default session budget (USD)
settings-default-budget-detail = Budget applied to each new session at start.
settings-spend-ceiling = Fleet spend ceiling (USD)
settings-spend-ceiling-detail = Total spend allowed across the fleet within the selected window.
settings-spend-window = Spend window (hours)
settings-spend-window-detail = Rolling time window for fleet spend enforcement.
settings-memory-budget = Fleet memory budget (MB)
settings-memory-budget-detail = Total sampled process memory available across the fleet.
settings-memory-cap = Per-session memory cap (MB)
settings-memory-cap-detail = Maximum sampled process memory a single session may consume.
settings-max-processes = Processes per session
settings-max-processes-detail = Maximum tracked child processes per session.
...
approvals-subtitle = Resolve blocked sessions without losing fleet context.
...
working-sets-subtitle = Save a focused fleet layout and restore it as one action.
...
working-sets-load-error = Could not read working sets: { $error }
...
search-subtitle = Find output across the live fleet and its bounded history.
