...
- [ ] P1 — Let a session carry its own agent credentials and config directory
  ...
  Touches: `crates/terminalai-core/src/environment.rs`, `launch.rs`, `crates/terminalai-app/src/preset.rs`, `web/index.html`, `web/src/main.js`, `README.md`
  Acceptance: deny-by-default is preserved — nothing new is inherited from the parent. A launch spec may name a config directory and an explicitly supplied credential set, per session and per preset; the README states which authentication modes are supported and which are not; and an agent that starts without a usable credential reports "no credential supplied" rather than reusing the expired-login banner. Gated on RESEARCH.md open question 2 (whether a redirected `CLAUDE_CONFIG_DIR` fragments `--resume`), which one live launch settles.
  Complexity: M
