...
- [ ] P0 — Stop offering a per-session budget the agent ignores
  Why: the launcher's budget field becomes `--max-budget-usd` in an **interactive** argv, and Claude Code enforces that flag only under `--print` — so the control implies a cap that never binds, which is the one failure mode a spend feature must not have.
  Evidence: `claude --help` on 2.1.170 states verbatim `--max-budget-usd <amount>  Maximum dollar amount to spend on API calls (only works with --print)`; https://code.claude.com/docs/en/cli-reference lists it under print-mode-only cost controls. Emitted at `crates/terminalai-core/src/launch.rs:526` from the field declared at `:246`, and pinned in `crates/terminalai-core/tests/fixtures/launch/claude-code-2.1.170.json:36`. The only `--print` path in the tree is the capability probe (`capabilities.rs:200`), which never carries a budget. Two things ride on the same flag: `registry/mod.rs:750` injects the fleet's `default_budget_usd` into the spec when the operator left the field blank, so the `TERMINALAI_DEFAULT_BUDGET_USD` admission setting has no other delivery mechanism; and `registry/mod.rs:614` sets `budget_enforced_agents: ["claude"]`, which `web/src/spendCeiling.js:50` renders as "A per-session budget is enforced for: claude". The fleet-wide ceiling and the transcript ledger are separate machinery and are unaffected.
  Touches: `crates/terminalai-core/src/launch.rs`, the agent manifest flag table, `crates/terminalai-core/src/admission.rs`, `crates/terminalai-core/src/registry/mod.rs` (the `default_budget_usd` injection and `budget_enforced_agents`), `crates/terminalai-core/tests/fixtures/launch/`, `web/src/main.js`, `web/src/spendCeiling.js`, `web/src/i18n/terminalai.ftl` (`spend-enforced-agents`), `README.md`
  Acceptance: an interactive launch never emits `--max-budget-usd`; either the field is refused per the `LaunchError::Unsupported` rule, or it is kept and relabelled to say that TerminalAI's ledger enforces it and the agent does not. `budget_enforced_agents` and the string it feeds say the same thing as the argv, `default_budget_usd` either gains a real delivery path or is documented as ledger-only, and a golden pins whichever was chosen.
  Complexity: S

- [ ] P0 — Measure the job, not the process
  Why: the per-session memory cap is enforced by the OS across the whole job, while the figure the row shows and admission spends against is read from one process — so a team can be killed by its own limit while the row reads "not limited" and the projection says there is room for another agent.
  Evidence: `private_bytes(pid)` (`crates/terminalai-core/src/process_tree.rs:263`) calls `GetProcessMemoryInfo` on the session's own pid only; `crates/terminalai-core/src/registry/sampling.rs:31` feeds exactly that into `session.memory_bytes`, which `admission.rs` spends against, including the `memory_limited` flag. Meanwhile `admission.rs:85` passes `session_memory_cap_bytes` into `JobLimits` and `process_tree.rs:116` sets it as `JOB_OBJECT_LIMIT_JOB_MEMORY` over every process in the job, so cap and reading disagree by construction. `JOBOBJECT_EXTENDED_LIMIT_INFORMATION` is already imported at `process_tree.rs:47` and the process *count* is already bounded at `:122`. The same single-process boundary applies to `set_background_priority` (`process_tree.rs:187`, called from `pty.rs:353`), which demotes only the lead when focus moves. Agent teams: https://code.claude.com/docs/en/agent-teams (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`, teammates are separate instances, cost scales linearly); concurrent-subagent cap of 20 added in Claude Code 2.1.216.
  Touches: `crates/terminalai-core/src/process_tree.rs`, `crates/terminalai-core/src/registry/sampling.rs`, `crates/terminalai-core/src/admission.rs`, `crates/terminalai-core/src/pty.rs`, `web/src/rowMarkup.js`
  Acceptance: a session's reported memory and its `memory_limited` flag are computed over every process in its job object, proven by a test that spawns a child under a real job and fails against the single-process reading. The row reports the process count the figure covers — this item owns that number; the two items below consume it rather than adding their own. Decide peak-versus-live explicitly (see `RESEARCH.md` open question 2) and record which was chosen and why. Whether `set_background_priority` should follow the job is answered explicitly, either way.
  Complexity: M
