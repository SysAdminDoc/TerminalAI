...
- [ ] P2 — Finish shell maintainability and replace brittle source contracts
  Command seam: live-session, terminal, queue, broadcast, replay, and session lifecycle commands now live in `crates/terminalai-app/src/session_commands.rs`; workspace layouts, search, worktrees, presets, projects, prompts, and folder pickers now live in `crates/terminalai-app/src/workspace_commands.rs`; the entry module only composes and registers them.
  Boundary evidence: `cargo test --locked -p terminalai-app` passes 62/62 after both extractions, and the build-time command manifest still validates the unchanged handler identifiers.
